          ...
          <a-col :span="20">
            <a-carousel arrows :afterChange="onChange" dotsClass="slick-dots slick-thumb">
              ...
              <div v-for="item in list" :key="item.id">
                <img
                  :src="item.imageUrl"
                  :style="{height:'460px',width:'1020px','min-width':'247px','min-height':'186px', 'object-fit': 'cover'}"
                />
              </div>
            </a-carousel>
          </a-col>
...
export default {
  data() {
    ...
    return {
      ...
      uploadUrl: `${this.$http.defaults.baseURL}/backend/file/upload`,
  ...
  methods: {
    ...
    confirm(item) {
      const url = `/backend/carousel/${item.id}`;
      this.$h
        .delete(url)
        .cb(data => {
          if (data.status === 0) {
